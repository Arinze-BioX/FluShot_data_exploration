...
plot(accuracy)
max_acc <- max(accuracy@y.values[[1]])
max_acc
```
...
accuracy <- performance(pred2, measure = "acc")
plot(accuracy)
max_acc <- max(accuracy@y.values[[1]])
max_acc
```
...
## Creating a simpler tree version for the PowerPoint Presentation:

```{r}
build_simple_tree <- function(x) {
  vac_com_h1n1 <- subset(x, select = -c(seasonal_vaccine))
  vac_com_seas <- subset(x, select = -c(h1n1_vaccine))
  
  tree_obj <- rpart(data = vac_com_h1n1, formula = h1n1_vaccine ~ ., method = "class", minsplit = 10, cp = 0.01)
  
  plot(tree_obj, margin = 0.1)
  title("Decision Tree H1N1")
  summary(tree_obj)
  text(tree_obj)
  ...
  tree_obj2 <- rpart(data = vac_com_seas, formula = seasonal_vaccine ~ ., method = "class", minsplit = 10, cp = 0.01)
  
  tree_obj2$variable.importance
  
  plot(tree_obj2, margin = 0.1)
  title("Decision Tree Seasonal")
  summary(tree_obj2)
  text(tree_obj2)
