...
trainingDF <- trainingDF %>% 
   mutate(age_group_num = as.numeric(age_group),
          ...
          income_poverty_num = as.numeric(income_poverty),
          marital_status_num = as.numeric(marital_status),
          rent_or_own_num = as.numeric(rent_or_own),
          employment_status_num = as.numeric(employment_status),
          hhs_geo_region_num = as.numeric(hhs_geo_region),
          census_msa_num = as.numeric(census_msa),
          employment_industry_num = as.numeric(employment_industry),
          employment_occupation_num = as.numeric(employment_occupation))
```
...
summary(missingnessDF)
p1 <- ggplot(missingnessDF, aes(x=cols, y=vals)) +
      geom_col(fill="#fb6097", color="white", alpha=0.75) +
     theme(plot.title = element_text(hjust = 0.5, color="#fb6097", size=10), axis.text = element_text(size=6, color ="gray"), axis.title = element_text(size=8), axis.text.x = element_text(angle = 45, hjust=1)) +
...
### Correlation Plot
```{r, fig.align="center"}
corr <- mixedCor(data=trainingDF,d=c("behavioral_antiviral_meds","behavioral_avoidance","behavioral_face_mask","behavioral_wash_hands","behavioral_large_gatherings","behavioral_outside_home","behavioral_touch_face","doctor_recc_h1n1","doctor_recc_seasonal","chronic_med_condition","child_under_6_months","health_worker","health_insurance","h1n1_vaccine","seasonal_vaccine","sex_num"), p=c("h1n1_concern","h1n1_knowledge","opinion_h1n1_vacc_effective","opinion_h1n1_risk","opinion_h1n1_sick_from_vacc","opinion_seas_vacc_effective","opinion_seas_risk","opinion_seas_sick_from_vacc","age_group_num","education_num","race_num","income_poverty_num","marital_status_num","rent_or_own_num","employment_status_num","census_msa_num", "household_adults","household_children"))
corrplot(corr$rho, order="hclust", method="circle", type="upper", tl.cex=0.7,number.cex=0.5)
```
...
```{r fig.align='center'}
p1 <- ggplot(trainingDF, aes(h1n1_concern)) +
