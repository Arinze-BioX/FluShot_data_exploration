...
bstSparse_a <- xgboost(data=rf.train.xg.h1n1, label=as.numeric(y.rf_train.h1n1), max.depth = 2, eta = 1, nthread = 2, nrounds = 20, objective = "binary:logistic", eval_metric="error",verbose = 0)
...
bstSparse_b <- xgboost(data=comp.train.xg.h1n1, label=as.numeric(y.comp_train.h1n1), max.depth = 2, eta = 1, nthread = 2, nrounds = 20, objective = "binary:logistic", eval_metric="error",verbose = 0)
...
X.comp_test <- comp.case.df[-train_ind, ] %>% subset(., select=-c(h1n1_vaccine, seasonal_vaccine))
...
X.unkn_test <- unknown.case.df[-train_ind, ] %>% subset(., select=-c(h1n1_vaccine, seasonal_vaccine))
...
X.mode_test <- mod.imp[-train_ind, ] %>% subset(., select=-c(h1n1_vaccine, seasonal_vaccine))
...
ggplot(importance_b[1:10,], aes(x = reorder(Feature, Importance), y = Importance))+ 
  geom_bar(stat="identity", fill="#191870") + 
  theme_classic() +
  coord_flip() +
  ...
  theme(plot.title = element_text(hjust = 1.3, size=11, face="bold", color="black"), axis.title = element_text(color="#191870", size=16), axis.text = element_text(color="black", size=11), axis.line = element_line(color="black"))
dev.off()

png(filename="comp_seasonal_Importance.png")
ggplot(importance_f[1:10,], aes(x = reorder(Feature, Importance), y = Importance))+ 
  geom_bar(stat="identity", fill="#191870") + 
  theme_classic() +
  coord_flip() +
  geom_hline(yintercept=0, color="#191870") +
